...
/**
 * Definition of API used for constructing Object Identifiers (as annotated using {@link JsonIdentityInfo}). Also
 * defines factory methods used for creating instances for serialization, deserialization.
 *
...
public abstract class ObjectIdGenerator<T>
    ...
    public final static class IdKey
        ...
        public String toString() {
            // hacked down to remove Hack.string_format
            return "[ObjectId: key=%s, type=%s, scope=%s" + key + " " +
    ...
    /**
     * Accessor that may be called (after verifying (via {@link #maySerializeAsObject()}) whether given name
     *
     ...
     *               because <code>JsonParser</code> is defined in `jackson-core`, and this package does not depend on
     *               it).
     * @since 2.5
     */
    ...
    /**
     * Accessor that needs to be overridden to return <code>true</code> if the Object Id may be serialized as JSON
     * Object; used by, for example, JSOG handling. The reason accessor is needed is that handling such Object Ids is
     * more complex and may incur additional buffering or performance overhead, avoiding of which makes sense for common
     * case of scalar object ids (or native object ids some formats support).
     * <p>
     * Default implementation returns <code>false</code>, so needs to be overridden by Object-producing generators.
     *
     * @since 2.5
     */
     ...
     * but can not be declared here as type itself (as well as call to this object) comes from databind package.
     *
