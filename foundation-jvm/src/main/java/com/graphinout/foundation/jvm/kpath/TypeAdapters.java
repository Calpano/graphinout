package com.graphinout.foundation.jvm.kpath;
...
import com.graphinout.foundation.pure.collections.MapMap;
import com.graphinout.foundation.pure.types.ITypeAdapter;
import com.graphinout.foundation.pure.types.ITypeAdapters;
 ...
 */
@GwtIncompatible("using getInterfaces of a type")
public class TypeAdapters implements ITypeAdapters {
     ...
     *
     * @param inputValue      the object to adapt.
     * @param targetType the class of the target type.
     * @param <T>        the source type.
     * @param <A>        the target type.
     ...
     */
    @Override
    public <T, A> @Nullable A adaptTo(T inputValue, Class<A> targetType) {
        Class<?> sourceType = inputValue.getClass();
        ITypeAdapter<T, A> adapter = _findAdapterFromTo(sourceType, sourceType, targetType, null, false);
        if (adapter == null) {
            return null;
        }
        return adapter.toAdapted(inputValue);
    }
     ...
     * @return the matching {@link ITypeAdapter}, or {@code null} if not found.
     */
    public <T, A> @Nullable ITypeAdapter<T, A> findAdapterFromTo(Class<?> sourceType, Class<A> targetType, boolean warnIfNoneFound) {
     ...
     *
     * @param inputType the class of the source type.
     * @param targetType the class of the target type.
     * @param adapter    the adapter to register.
     * @param <T>        the source type.
     * @param <A>        the target type.
     */
    public <T, A> void register(Class<T> inputType, Class<A> targetType, ITypeAdapter<T, A> adapter) {
        adapters.put(inputType, targetType, adapter);
    }
