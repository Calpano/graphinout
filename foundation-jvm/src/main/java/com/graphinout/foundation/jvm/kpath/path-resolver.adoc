...
* `GraphmlResult`: Encapsulates the result of a path resolution.
...
| Map 1 | foo | Get the value associated with key `annotations` from map-like. If not a map, return nothing.
...
The `GraphmlResult` class holds the outcome of a resolution. It contains:
...
* `ITypeAdapter`: A functional interface that converts an object of type `I` to an adapted type `O`.
