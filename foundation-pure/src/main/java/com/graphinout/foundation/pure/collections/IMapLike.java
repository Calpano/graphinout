 ...
 * <p>
 * This allows, e.g., a {@code PathResolver} to navigate objects that are not standard {@link Map}s but can be
 * treated as such.
 */
...
public interface IMapLike {
    ...
    /**
     * Creates a trivial wrapper for a standard {@link Map}.
     *
    ...
    static IMapLike ofMap(Map<String, ?> javaMap) {
        return of(new ArrayList<>(javaMap.keySet()), javaMap::get);
    }
    ...
    static IMapLike ofProperty(String key, Supplier<Object> supplier) {
        return of(Java9.List.of(key), k -> k.equals(key) ? supplier.get() : null);
    }
