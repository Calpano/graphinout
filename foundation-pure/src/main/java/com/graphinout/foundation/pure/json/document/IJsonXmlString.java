...
public interface IJsonXmlString extends IJsonPrimitive {
    ...
    static IJsonXmlString ofJsonValue(IJsonValue jsonValue) {
        switch (jsonValue.jsonType()) {
            case XmlString:
                return (IJsonXmlString) jsonValue;
            case Object:
                return ofJsonObject(jsonValue.asObject());
            case String:
                return IJsonXmlString.ofPlainString(jsonValue.factory(), jsonValue.asString());
            case Boolean:
            case Number:
                return IJsonXmlString.ofPlainString(jsonValue.factory(), jsonValue.toJsonString());
            default:
    ...
    default Map<String, Object> toJaJsonMap() {
        return JaJson.createMap().putMaybe(XML, rawXmlString()).putMaybe(XML_SPACE, xmlSpace(), x -> x.jsonStringValue).build();
    }
