...
public class JsonTypes {
    ...
    public static Class<?> strictestType(Number n) {
        if (n instanceof BigDecimal) {
            return strictestType((BigDecimal) n);
        } else if (n instanceof BigInteger) {
            return strictestIntegerType((BigInteger) n);
        } else if (n instanceof Double) {
            if (!Double.isFinite((Double) n)) {
                return Double.class;
            }
            return strictestType(BigDecimal.valueOf((Double) n));
        } else if (n instanceof Float) {
            if (!Float.isFinite((Float) n)) {
                return Float.class;
            }
            return strictestType(BigDecimal.valueOf(n.doubleValue()));
