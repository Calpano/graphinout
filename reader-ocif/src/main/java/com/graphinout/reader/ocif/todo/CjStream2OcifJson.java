...
public class CjStream2OcifJson extends BaseCjOutput implements ICjStream {
    ...
    public CjStream2OcifJson() {
        this(null);
    }


    public CjStream2OcifJson(@Nullable Consumer<String> onDone) {
        this.onDone = onDone;
    }
    ...
    public void documentStart(ICjDocumentChunk cjDoc) {
        ...
        cjDoc.data(data -> ifPresentAccept(data.jsonValue(), IJsonValue::asObjectOrNull, o -> {
            o.getMaybeAs(_OCIF, IJsonValue::asObjectOrNull, ocifObject -> {
                ifPresentAccept(ocifObject.get("schemaUri"), v -> root.setProperty(_OCIF, v));
    ...
    public void edgeStart(ICjEdgeChunk edge) {
        ...
        edge.data(data -> ifPresentAccept(data.jsonValue(), IJsonValue::asObjectOrNull, eo -> //
                eo.getMaybeAs(_OCIF, IJsonValue::asObjectOrNull, ocif -> //
    ...
    public void nodeStart(ICjNodeChunk node) {
        ...
        if (dv != null && dv.isObject()) {
            ...
            if (od.hasProperty(_OCIF)) {
                IJsonValue ocifVal = od.get(_OCIF);
